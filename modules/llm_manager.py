...
class LLMManager:
    def __init__(self, config):
        ...
        if not api_key:
            logging.warning(
                "GEMINI_API_KEY not set; responses will use fallback text"
            )
            self.client = None
        else:
        ...
        logging.info(
            f"LLM Manager initialized in {self.current_mode} mode (model={self.model_name})"
        )
    ...
    def _fallback_reply(self, prompt: str) -> str:
        ...
        if any(x in p for x in ["hello", "hi", "hey"]):
            return (
                "Hello! I'm online. Add GEMINI_API_KEY to your .env for full AI responses."
            )
        return (
            "Temporarily unable to reach Gemini. Please set GEMINI_API_KEY and try again."
        )
